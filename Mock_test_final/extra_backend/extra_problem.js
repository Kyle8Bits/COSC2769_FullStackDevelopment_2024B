const { customers, products, orders } = require("../backend/data");
...
const OrdersType = new GraphQLObjectType({
    ...
    fields: {
      ...
      order_amount: {
        type: GraphQLFloat,
        resolve: (order) => {
          const productsInOrder = products.filter(p => p.id === order.product_id);
          return productsInOrder.reduce((acc, product) => {
            const price = product.price * (1 - (product.percent || 0) / 100);
      ...
      products: {
        ...
        resolve: (order, args) => {
          return products
            .filter((product) => product.id === order.product_id)
            .filter((product) => {
              const discountedPrice = product.price * (1 - (product.percent || 0) / 100);
              return (
                (!args.min_price || discountedPrice >= args.min_price) &&
                (!args.max_price || discountedPrice <= args.max_price)
              );
            });
        },
      },
  ...
  const CustomerType = new GraphQLObjectType({
    ...
    fields: {
      ...
      orders: {
        ...
        args: {
          min_quantity: { type: GraphQLInt },
          max_quantity: { type: GraphQLInt },
        },
        resolve: (customer, args) => {
          return orders
            ...
            .filter((order) => {
              return (
                (!args.min_quantity || order.quantity >= args.min_quantity) &&
                (!args.max_quantity || order.quantity <= args.max_quantity)
              );
            });
        },
      },
